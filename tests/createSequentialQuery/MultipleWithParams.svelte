...
<button onclick={() => loadMore1()}>Load More 1</button>

<div>Loading 1: {query1.loading}</div>
<div>Error 1: {query1.error}</div>
<div>Has More 1: {query1.hasMore ? 'Yes' : 'No'}</div>
...
<button onclick={() => loadMore2()}>Load More 2</button>

<div>Loading 2: {query2.loading}</div>
<div>Error 2: {query2.error}</div>
<div>Has More 2: {query2.hasMore ? 'Yes' : 'No'}</div>
